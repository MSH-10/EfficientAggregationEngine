...
public class Solution
{
    public static Dictionary<string, double>[] DoAggregations(double[,] input)
    {
        ...
        HashSet<double>[] distinctSets = new HashSet<double>[cols]; // For distinct count
        ...
        for (int i = 0; i < cols; i++)
        {
            ...
            distinctSets[i] = new HashSet<double>(); // Initialize HashSet for each column
        }
        ...
        for (int i = 0; i < rows; i++)
        {
            for (int j = 0; j < cols; j++)
            {
                ...
                // Sum and count distinct values using HashSet
                sums[j] += value;
                counts[j]++;
                distinctSets[j].Add(value); // Add to HashSet for distinct counting
            }
        }
        ...
        for (int j = 0; j < cols; j++)
        {
            ...
            result[j]["COUNT DISTINCT"] = distinctSets[j].Count; // Exact distinct count
        }
